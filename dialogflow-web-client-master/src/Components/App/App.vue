...
<main id="app">
    ...
    <section class="container chat-container">
        ...
        <section v-else aria-live="polite">
            <div v-for="m in messages" :key="m.responseId" class="message">
                ...
                <section v-if="!m.queryResult.webhookPayload">
                    <div v-for="(component, id) in m.queryResult.fulfillmentMessages" :key="id" class="component">
                        ...
                        <!-- <Bubble v-if="component.simpleResponses" :text="component.simpleResponses.simpleResponses[0].displayText || component.simpleResponses.simpleResponses[0].textToSpeech" /> -->

                        <!-- Card (https://cloud.google.com/dialogflow/docs/reference/rest/v2beta1/projects.agent.intents#Card) -->
                        <Card v-if="component.card" :title="component.card.title" :subtitle="component.card.subtitle" :image-uri="component.card.imageUri">
                        ...
                        <Card v-if="component.basicCard" :title="component.basicCard.title" :subtitle="component.basicCard.subtitle" :image-uri="component.basicCard.image.imageUri" :image-title="component.basicCard.image.accessibilityText"
                        ...
                        <Carousel v-if="component.carouselSelect">
                            <Card v-for="item in component.carouselSelect.items" :key="item.info.key" :title="item.title" :image-uri="item.image.imageUri" :image-title="item.image.accessibilityText" :text="item.description"
                        ...
                        <List v-if="component.listSelect" :title="component.listSelect.title" :subtitle="component.listSelect.subtitle">
                            <ListItem v-for="item in component.listSelect.items" :key="item.title" :title="item.title" :description="item.description" :image-uri="item.image.imageUri" :image-title="item.image.accessibilityText"
                                @click.native="send(item.info.key)" />
                        </List>
                ...
                <section v-if="m.queryResult.webhookPayload && m.queryResult.webhookPayload.google">
                    <div v-for="(component, id) in m.queryResult.webhookPayload.google.richResponse.items" :key="id" class="component">
                        ...
                        <Bubble v-if="component.simpleResponse" :text="component.simpleResponse.displayText || component.simpleResponse.textToSpeech" />

                        <!-- Basic card (https://developers.google.com/actions/assistant/responses#basic_card) -->
                        <Card v-if="component.basicCard" :title="component.basicCard.title" :subtitle="component.basicCard.subtitle" :image-uri="component.basicCard.image.url" :image-title="component.basicCard.image.accessibilityText"
                            :text="component.basicCard.formattedText">
                        ...
                        <List v-if="component.carouselBrowse">
                            <ListItem v-for="item in component.carouselBrowse.items" :key="item.title" :uri="item.openUrlAction.url" :title="item.title" :description="item.description" :footer="item.footer" :image-uri="item.image.url"
                        ...
                        <div v-if="component.mediaResponse && component.mediaResponse.mediaObjects">
                            <Media v-for="media in component.mediaResponse.mediaObjects" :key="media.name" :name="media.name" :description="media.description" :icon-uri="media.icon.url" :icon-title="media.icon.accessibilityText"
                                :uri="media.contentUrl" />
                        </div>

                        <!-- Table cards (https://developers.google.com/actions/assistant/responses#table_cards) -->
                        <TableCard v-if="component.tableCard" :title="component.tableCard.title" :subtitle="component.tableCard.subtitle" :image-uri="component.tableCard.image.url" :image-title="component.tableCard.image.accessibilityText"
                    ...
                    <div v-for="(component, id) in m.queryResult.webhookPayload.google.systemIntent" :key="id" class="component">
                        ...
                        <List v-if="component.listSelect" :title="component.listSelect.title" :subtitle="component.listSelect.subtitle">
                            <ListItem v-for="item in component.listSelect.items" :key="item.optionInfo.key" :title="item.title" :description="item.description" :image-uri="item.image.url" :image-title="item.image.accessibilityText"
                                @click.native="send(item.optionInfo.key)" />
                        </List>

                        <!-- Carousel (https://developers.google.com/actions/assistant/responses#carousel) -->
                        <Carousel v-if="component.carouselSelect">
                            <Card v-for="item in component.carouselSelect.items" :key="item.optionInfo.key" :title="item.title" :image-uri="item.image.url" :image-title="item.image.accessibilityText" :text="item.description"
                                @click.native="send(item.optionInfo.key)" />
                        </Carousel>
                    </div>
                </section>
    ...
    <ChatInput @submit="send">
        ...
        <span v-if="suggestions.text_suggestions">
            <Suggestion v-for="(suggestion, index) in suggestions.text_suggestions" :key="index" :title="suggestion" @click.native="send(suggestion)" />
        </span>
        ...
        <!-- Link suggestion chips
            ...
            -->
        <Suggestion v-if="suggestions.link_suggestion" :title="suggestions.link_suggestion.destinationName" :url="suggestions.link_suggestion.uri || suggestions.link_suggestion.url" />
    </ChatInput>
</main>
</template>
...
export default {
    ...
    data() {
        return {
            ...
            loading: false,
            error: null,
            client: new Client(this.config.app.gateway)
        }
    },
    computed: {
    ...
    methods: {
        send(q) {
            const request = {
                ...
                live: this.live, // param indicating if session is live experiment
                queryInput: {
                    text: {
                        text: q,
